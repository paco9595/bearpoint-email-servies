...
export default function DashboardProjectsPage() {
  ...
  const [organizations, setOrganizations] = useState<{ id: string, name: string, project: any[] }[]>()

  useEffect(() => {
    const getOrganizationData = async () => {
      const { data, error } = await supabase.from('organization').select('*, project(*)')
      if (!error) {
    ...
    const project = supabase
      ...
      .on(
        ...
        (payload: any) => {
          ...
          setOrganizations((old) => old?.map(organization => {
          ...
          <div className="min-h-44 h-full my-3 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 w-full gap-3 md:gap-5 ">
            {projects.map(
              ({
                ...
                <Card key={id} className="rounded-[5px] px-5 py-6 border-border bg-background">
            ...
            {projects.length == 0 && (
              <div className="text-sm text-muted-foreground">Not projects jet</div>
            )}
          </div>
        </div>
      ))}
